...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, UUID> {
    ...
    @Query("""
            ...
            WHERE r.expiresAt < :now
               OR (r.revoked = true AND r.revokedAt IS NOT NULL AND r.revokedAt < :revokedCutoff)
            """)
    int deleteExpiredOrRevoked(@Param("revokedCutoff") Instant revokedCutoff, @Param("now") Instant now);
}
